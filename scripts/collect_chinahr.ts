...
const DETAIL_EVAL = `(function(){
  ...
  return JSON.stringify({position:pos, company:co, salary:salary, city:city, jd:jd.slice(0,5000), textLen:flat.length});
})()`;
...
(async () => {
  ...
  await sleep(4000);
  for (let i = 0; i < 5; i++) { await ex('eval', { script: 'window.scrollBy(0,1200); "ok"' }); await sleep(500); }

  const r = await ex('eval', { script: LIST_EVAL });
  const cards: string[] = (r.data as any) || [];
  console.log(`[列表] 收集到 ${cards.length} 个详情链接`);

  const pool: any[] = [];
  const target = cards.slice(0, LIMIT);
      ...
      try {
        ...
        if (n2.ok) {
          ...
          if (EXCLUDE.some((e) => s.includes(e)) && !KEEP.some((k) => s.includes(k))) {
            console.log(`[${i + 1}/${target.length}] ⛔ 非技术岗跳过：${String(item.position).slice(0, 22)}`);
            continue;
          }
          if (CITY && !(item.city || '').includes(CITY)) {
            console.log(`[${i + 1}/${target.length}] ⛔ 城市不符(${item.city})跳过：${String(item.position).slice(0, 22)}`);
            continue;
          }
          console.log(`[${i + 1}/${target.length}] ✅ ${String(item.position).slice(0, 22)} | ${String(item.company || '').slice(0, 16)} | ${item.salary || '-'} | ${item.city || '-'}`);
        }
      } catch (e: any) {
  ...
  if (!inserted) console.log('    （中华英才网无 URL 搜索，推荐列表多为非技术/外地岗；可换 --city 或人工在窗口搜索后采集）');
