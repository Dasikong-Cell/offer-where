...
### 9.7 已知边界（不影响中文 Windows，但值得知道）

**中文名脚本在 zip 内以 GBK 字节存储、未置 UTF-8 标记。** 取证：`创建桌面快捷方式.bat` 在归档里的原始名字字节是
`b4b4bda8d7c0c3e6bfecbdddb7bdcabd2e626174`，按 GBK 解码为 `创建桌面快捷方式.bat`；`flag_bits & 0x800`（UTF-8 标记）**未置位**
（这是 Windows 自带 `tar`/bsdtar 的默认行为：按本机 ANSI 代码页写名字）。

| 环境 | 结果 |
|---|---|
| 中文 Windows（本工具的目标平台） | ✅ 资源管理器 / .NET `ZipFile` / 7-Zip 都按 GBK 解码 → 文件名正常 |
| 英文等非中文 Windows | ⚠️ 按 cp1252 解码 → 该**可选**辅助脚本名会显示为乱码（文件本体无损，仍可双击） |

- 影响面小：该脚本只是「在桌面建一个快捷方式」，不是启动链路的一部分（启动永远是双击 `start_all.bat`），README 的安装步骤也没依赖它。
- 若将来要面向非中文 Windows：**最省事的修法是把它改成 ASCII 名**（如 `create_desktop_shortcut.bat`），中文名只留在 README/控制台文案里；改 zip 内编码需要重写归档，代价不值得。
- 另注：`pack.ps1` 的反向断言（`$shippedDev`）在**英文 runner（CI）**上比较 CJK 名会因控制台代码页不等而匹配不上 —— 但真正的剔除动作由 `.NET` 文件名比较完成（可靠），断言只是双保险，故 CI 不会因此误判。
